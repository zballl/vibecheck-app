...
if prompt := st.chat_input("Type your mood here..."):
    ...
    st.rerun() # Force a rerun to trigger the logic below

# CHECK IF LAST MESSAGE WAS USER AND NEEDS REPLY
# This handles both Button Clicks AND Typing!
if st.session_state.messages and st.session_state.messages[-1]["role"] == "user":
    with st.chat_message("assistant", avatar="🎧"):
        with st.spinner("Curating tracks..."):
            last_user_msg = st.session_state.messages[-1]["content"]
            response = get_vibe_check(last_user_msg)
